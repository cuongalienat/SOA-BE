  ...
  try {
    ...
    if (!finalLat || !finalLng) {
      ...
      if (!coords) {
        throw new ApiError(
          400,
          "Không tìm thấy địa chỉ này trên bản đồ. Vui lòng ghi rõ hơn."
        );
      }
    ...
    for (const itemData of items) {
      ...
      if (!dbItem)
        throw new ApiError(404, `Món ${itemData.item} không tồn tại.`);
...
export const updateOrderStatusService = async (
  ...
  if (!allowedStatuses || !allowedStatuses.includes(normalizedStatus)) {
    throw new ApiError(
      403,
      `Bạn không có quyền chuyển trạng thái đơn hàng sang "${newStatus}".`
    );
  }
...
export const cancelOrderService = async (orderId, userId) => {
  ...
  try {
    ...
    if (order.user.toString() !== userId)
      throw new ApiError(StatusCodes.FORBIDDEN, "Không có quyền hủy.");
    if (order.status !== "Pending" && order.status !== "Confirmed") {
      throw new ApiError(
        StatusCodes.BAD_REQUEST,
        "Không thể hủy đơn khi đã có tài xế nhận."
      );
    }
    ...
    if (order.delivery) {
      await Delivery.findByIdAndUpdate(order.delivery, {
        status: "CANCELLED",
        $push: {
          trackingLogs: { status: "CANCELLED", note: "Khách hàng hủy đơn" },
        },
      }).session(session);
    }
